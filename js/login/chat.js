...
function computer(){
    AllFriends.forEach((item,index)=>{
        ...
        let li2 = document.querySelectorAll(".friend-li2")
        li2.forEach(item=>{
            item.onclick = function(){
                let tagname = item.querySelector(".friend-name2")
                addblock.append(tag(tagname))
                item.style.display= 'none'

                let img = item.querySelector(".friend-img2").src
                let name = item.querySelector(".friend-name2").innerHTML
                let line = item.querySelector(".friend-line2").innerHTML
                ...
                check.onclick = function(){
                    ...
                    div.innerText = groupname.value
                    friendarray.forEach(item=>{
                        div.append(getfriend2(item.img,item.name,item.line))
                    })
                    friendgroup.append(div)
        ...
        friend.forEach((item=>{
            ...
            item.onmousedown = function(e){
                if(e.which == 3){
                    //自定義滑鼠右鍵展開的菜單
                item.oncontextmenu = function (e1) { 
